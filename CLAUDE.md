...
- `src/app/methodology/jaiyq-mri` + `public/JAIYQ-MRI-adistemesi.pdf` —
  🦟 МАСА МОДЕЛІНІҢ ТОЛЫҚ ӘДІСТЕМЕСІ: формулалар, параметрлердің негізі,
  жеті әлемдік модельмен салыстыру, уникальділіктің шыншыл бағасы, не
  жетіспейтіні + шешу жолы. Модель өзгерсе — бет ПЕН PDF екеуі де жаңарады
