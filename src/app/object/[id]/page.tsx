  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-8 print:max-w-none print:py-0">
      ...
      <div className="mb-6 rounded-xl border border-amber-400/30 bg-amber-500/[0.07] p-4">
        ...
        <p className="text-[12px] leading-relaxed text-amber-100/90">{d.causalityWarning}</p>
      </div>
      ...
      <Section n={1} title={tr("Экологиялық көрсеткіштер және заңға сәйкестік")} icon={Scale}>
            ...
            {d.compliance.results.map((r) => (
              <div key={r.indicatorId} className="rounded-lg border border-white/10 bg-white/[0.02] p-2.5">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-[12px] text-neutral-100">{r.name}</span>
                  <span className="ml-auto text-[13px] font-semibold text-white">
                    {r.value == null ? "—" : `${r.value} ${r.unit}`}
                  </span>
                </div>
                <p className="mt-1 text-[10px] text-neutral-400">{r.summary}</p>
                {r.checks.length > 0 && (
                  <div className="mt-1.5 flex flex-wrap gap-x-3 gap-y-0.5 border-t border-white/5 pt-1.5 text-[10px] text-neutral-500">
                    {r.checks.map((c, i) => (
                      <span key={i}>
        ...
        <p className="mt-2 text-[10px] leading-relaxed text-amber-200/70">⚖ {d.disclaimer}</p>
      </Section>
      ...
      <Section n={d.nearbyFlares.length > 0 ? 4 : 3} title={tr("Дәлелдер тізбегі")} icon={FileText}>
        <div className="space-y-2">
          {d.evidence.map((e, i) => (
            <div key={i} className="rounded-lg border border-white/10 bg-white/[0.02] p-2.5 text-[11px]">
              ...
              <div className="mt-0.5 text-[10px] text-amber-200/70">⚠ {e.note}</div>
            </div>
          ))}
        </div>
      </Section>
      ...
      <Section n={d.nearbyFlares.length > 0 ? 5 : 4} title={tr("Жалпы ақпарат")} icon={Factory}>
        <dl className="grid gap-x-6 gap-y-1 text-[11px] sm:grid-cols-[max-content_1fr]">
          <dt className="text-neutral-500">{tr("Түрі")}</dt>
          <dd className="text-neutral-200">{d.kind}</dd>
          <dt className="text-neutral-500">{tr("Координата")}</dt>
          <dd className="text-neutral-200">
            {d.coords.lat.toFixed(5)}, {d.coords.lng.toFixed(5)}
          </dd>
          <dt className="text-neutral-500">{tr("Координата дәлдігі")}</dt>
          ...
          <div className="mt-1 space-y-1">
            {Object.entries(d.general.emissionProfile).map(([k, v]) => (
              <div key={k} className="flex items-center gap-2">
                <span className="w-44 shrink-0 text-[11px] text-neutral-300">
                  {PROFILE_KZ[k] ?? k}
                </span>
      ...
      <footer className="mt-6 border-t border-white/10 pt-4 text-[10px] leading-relaxed text-neutral-500">
        <p>
