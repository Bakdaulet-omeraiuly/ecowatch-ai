  ...
  try {
    ...
    return NextResponse.json({
      ...
      coverage: {
        ...
        gapHours,
        // ⚠️ ТӘУЛІКТІК cron (Vercel Hobby жоспарында cron тәулігіне бір рет
        // қана жүреді). Бірақ әр жүгіріс өткен 48 САҒАТТЫҢ ӘР САҒАТЫН
        // тексереді, сондықтан тәуліктік кесте де толық қамту береді:
        // үзіліс болса, кейінгі жүгіріс оны артқа қарай ЖАБАДЫ.
        //
        // Демек «үзілген» деп есептеу шегі де тәулікпен өлшенеді. Бұрын
        // ол 2 сағат болатын — сағат сайынғы cron-ға есептелген еді, ал
        // тәуліктік кестеде ол ӘРҚАШАН «үзілген» деп қызарып тұратын.
        interrupted: gapHours == null || gapHours > 26,
        note:
          lastRun == null
            ? "Тексеру әлі бір рет те жүрмеген — журналдағы бос орын «асу болмады» дегенді БІЛДІРМЕЙДІ."
            : gapHours != null && gapHours > 26
              ? `Соңғы тексеру ${gapHours} сағат бұрын (кесте — тәулігіне бір рет). ` +
                `Бақылау үзілген: 48 сағаттық артқа тексеру де жаппайтын аралық болуы мүмкін.`
              : "Бақылау үзіліссіз жүріп тұр. Тексеру тәулігіне бір рет жүреді, " +
                "әр жүгіріс өткен 48 сағаттың әр сағатын бөлек қарайды.",
      },
      note:
