...
// (5×5 = 25) + қаланың 65 нүктесі + қала сыртындағы 7 елді мекен = 97 —
// шек ішінде, БІРАҚ бос орын небәрі 3. Жаңа нүкте қосқанда осыны тексеру
// керек; 100-ден асса сұранысты екіге бөлу қажет.
...
// ─────────────────────────────────────────────────────────────────────────
// ҚАЛА СЫРТЫНДАҒЫ ЕЛДІ МЕКЕНДЕР — Атырау облысының аудан орталықтары мен
// ірі кенттері.
//
// НЕГЕ БӨЛЕК: жоғарыдағы тізілім — қала ІШІНДЕГІ айырманы көрсетуге
// арналған. Ал облыстық 5×5 тор аты жоқ, тек координаталық ұяшық береді:
// «Мақатта индекс қандай?» деген сұраққа жауап бермейді. Мына нүктелер
// сол олқылықты жабады — аты бар, орны нақты.
//
// ⚠️ КООРДИНАТА ДЕРЕККӨЗІ (ойдан жазылған сан ЖОҚ):
//   dr5hn/countries-states-cities-database → contributions/cities/KZ.json
//   (GeoNames негізіндегі ашық база, CC BY 4.0). Әрқайсысы сол файлдағы
//   `latitude`/`longitude` мәні, өзгертілмеген.
//
// ⚠️ НЕГЕ ТЕК ЖЕТЕУІ: Еркінқала, Дамба, Томарлы, Талғайран сияқты шағын
// ауылдар осы базада ЖОҚ. Оларды жадтан жазу — ойдан дерек жасау болар
// еді: нүкте 3–5 км жылжыса, MRI мүлдем басқа жерді өлшейді. Сондықтан
// қосылмады. Кейін нақты координата табылса (Google Maps-тан алып берсе
// де болады), осында тіркеу керек.
export const ATYRAU_OBLAST_SETTLEMENTS: DistrictPoint[] = [
  // ── Аудан орталықтары ──
  { lat: 47.6667, lng: 51.5833, name: "Махамбет" },   // Махамбет ауданы
  { lat: 48.5500, lng: 51.7833, name: "Индербор" },   // Индер ауданы
  { lat: 47.6500, lng: 53.3167, name: "Мақат" },      // Мақат ауданы
  { lat: 46.9531, lng: 54.0198, name: "Құлсары" },    // Жылыой ауданы

  // ── Ірі кенттер ──
  { lat: 47.5272, lng: 52.9811, name: "Доссор" },     // Мақат ауданы
  { lat: 47.2414, lng: 52.9411, name: "Байшонас" },
  { lat: 46.4353, lng: 53.4864, name: "Қаратон" },    // Жылыой ауданы
];
