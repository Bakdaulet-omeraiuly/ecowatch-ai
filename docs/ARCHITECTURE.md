...
### 1.9 Асу ФИКСАЦИЯЛАНАДЫ, кэшпен бірге жоғалмайды

Сәйкестік сұраныс кезінде есептеліп, 30 минуттық кэшке салынады. Түнгі
асу таңертең қарағанда жоғалып кететін. Енді `/api/fixate` сағат сайын
жүріп, әр асуды `exceedances` кестесіне жазады.

Екі уақыт БӨЛЕК: `observed_hour` — деректің өз сағаты, `recorded_at` —
тіркеу сәті. Шатастыруға болмайды.

⚠️ `fixation_runs` кестесі — журналдағы БОС кезең «асу болмады» ма, әлде
«тексеру жүрмеді» ме, соны ажыратады. Онсыз бос журнал жалған тыныштық
береді.

### 1.10 Шкала бір ғана жерде анықталады
...
| `supabase/exceedances.sql` | Асу журналы + тексеру жүгірістері (кесте схемасы) | `/api/fixate`, `/api/exceedances` |
...
| Ластану көзі | `lib/pollutionSource.ts` + `lib/dispersion.ts` | Жеңілдетілген CWT + Pasquill–Gifford дисперсия конусы. Тек Атырау |
...
- **L2 динамикасы дерекқорсыз жасалды** — күйді Supabase-те сақтап, күн
  ...
  осылай есептеледі.
- **Асу журналы қосылды** — бұрын асу кэш өткен соң жоғалатын, ал
  прокуратура үшін керегі дәл «қашан, қандай мән, қандай нормадан».
  Сағат сайынғы cron (`/api/fixate`) → Supabase. Жазу тек
  SUPABASE_SERVICE_ROLE_KEY-мен (anon-ға RLS рұқсат бермейді).
- **Дашбордтағы қайшылық түзетілді** — жоғарыда «НОРМА АСҚАН 0», төменде
  PM₂.₅ қызыл «НОРМА АСҚАН» болып тұратын. Себебі: санауыш `kzViolations`
  (тек ҚР актісі), ал белгі `worst`-тан (WHO/EU да кіреді) шығатын. Енді
  екеуі бөлек: «ҚР нормасы асқан» мен «WHO/EU эталонынан асқан».
- **Ластану көзі: төрт түзету (аудиттен кейін)** — (1) жел бүкіл торға бір
  нүктеден таралатын, енді әр қабылдағыштың өз желі (тор ~130 × 120 км,
  Каспий бризі); (2) конус ені тек жел жылдамдығынан шығатын, енді
  Pasquill орнықтылық класы + Briggs σy — 10 км-де D класы 6,5°, бұрын
  14–30° болатын; (3) осьтік кему `exp(−d/30)` гаусс шлейфінің түрі емес
  еді, енді 1/(π·σy·σz·u); (4) 🔴 карта мен мәтін сәйкес келмейтін —
  сервер бұлтты сағаттық болжам желімен жылжытатын, ал клиент оны елемей
  ағымдағы желмен қайта есептейтін (жел бұрылғанда 20 км алшақтық).
- **Конус пен пуф — екі түрлі модель** деп ашық жазылды. Конус: тұрақты
  күй («жел осылай тұрса»). Шеңбер: Лагранж пуфы («қазір шыққан ауа N
  сағаттан кейін»). Олар әртүрлі бағыт көрсетуі — қате емес.
- **Оқиға жоқ кезде конус СҰР үзік сызық** — бұрын қызыл болатын. Аты
  аталған кәсіпорыннан қызыл шлейф салу оқиға жоқ кезде беделдік/құқықтық
  тәуекел тудырады.
